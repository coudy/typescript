...
$(BUILT_LOCALTEST)\run.js: unit-tests-dependencies
	...
	$(STRC_LOCAL) -noresolve -target es5 $(BUILT_LOCALTEST)\typescriptServices.d.ts $(CSRC)\io.ts $(CSRC)\optionsParser.ts $(HSRC)\exec.ts  $(HSRC)\diff.ts $(HSRC)\harness.ts $(HSRC)\baselining.ts $(HSRC)\fourslash.ts $(HSRC)\dumpAST-baselining.ts $(HSRC)\external\json2.ts $(RSRC)\runnerbase.ts $(RSRC)\runnerfactory.ts $(RSRC)\compiler\runner.ts $(RSRC)\compiler\sourcemapRunner.ts $(RSRC)\fourslash\fsrunner.ts $(RSRC)\projects\runner.ts $(RSRC)\unittest\unittestrunner.ts $(HSRC)\runner.ts -out $(BUILT_LOCALTEST)\run.js
