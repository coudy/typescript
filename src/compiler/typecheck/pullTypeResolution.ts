...
module TypeScript {
    ...
    export class PullTypeResolver {
            ...
            if (varDecl.typeExpr) {
                ...
                if (!typeExprSymbol) {
                    ...
                    if (typeExprSymbol.isContainer()) {
                        ...
                        if (!instanceSymbol) {
                            diagnostic = context.postError(this.unitPath, varDecl.minChar, varDecl.getLength(), DiagnosticCode.Tried_to_set_variable_type_to_uninitialized_module_type__0__, [typeExprSymbol.toString()], decl);
        ...
        public resolveStatementOrExpression(expressionAST: AST, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext): SymbolAndDiagnostics<PullSymbol> {
            switch (expressionAST.nodeType) {
                ...
                case NodeType.ObjectCreationExpression:
                    return SymbolAndDiagnostics.fromSymbol(this.resolveNewExpression(<CallExpression>expressionAST, inContextuallyTypedAssignment, enclosingDecl, context));
        ...
        public resolveCallExpression(callEx: CallExpression, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext, additionalResults?: PullAdditionalCallResolutionData): PullSymbol {
            ...
            if (this.isAnyOrEquivalent(targetTypeSymbol)) {
                ...
                if (callEx.typeArguments) {
                    diagnostic = context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Untyped_function_calls_may_not_accept_type_arguments, null, enclosingDecl);
                    return this.getNewErrorTypeSymbol(diagnostic);
                }
            ...
            if (callEx.target.nodeType === NodeType.SuperExpression) {
                ...
                if (targetTypeSymbol.isClass()) {
                    ...
                    diagnostic = context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Calls_to__super__are_only_valid_inside_a_class, null, enclosingDecl);
                    return this.getNewErrorTypeSymbol(diagnostic);
                }
            }
        ...
        public resolveNewExpression(callEx: CallExpression, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext, additionalResults?: PullAdditionalCallResolutionData): PullSymbol {
            ...
            if (this.isAnyOrEquivalent(targetTypeSymbol)) {
                this.setSymbolAndDiagnosticsForAST(callEx, SymbolAndDiagnostics.fromSymbol(targetTypeSymbol), context);
                return targetTypeSymbol;
            }
            ...
            if (constructSignatures.length) {
                ...
                if (!signature) {
                    diagnostic = context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Could_not_select_overload_for__new__expression, null, enclosingDecl);
                ...
                if (usedCallSignaturesInstead) {
                    if (returnType != this.semanticInfoChain.voidTypeSymbol) {
                        diagnostic = context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Call_signatures_used_in_a__new__expression_must_have_a__void__return_type, null, enclosingDecl);
                        return this.getNewErrorTypeSymbol(diagnostic);
                    }
                    else {
                        returnType = this.semanticInfoChain.anyTypeSymbol;
            ...
            diagnostic = context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Invalid__new__expression, null, enclosingDecl);
