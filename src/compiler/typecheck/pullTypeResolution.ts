...
module TypeScript {
    ...
    export class PullTypeResolver {
        ...
        public getVisibleMembersFromExpression(expression: AST, enclosingDecl: PullDecl, context: PullTypeResolutionContext): PullSymbol[]{
            ...
            var lhs = this.resolveStatementOrExpression(expression, false, enclosingDecl, context).symbol;
        ...
        private resolveImportDeclaration(importStatementAST: ImportDeclaration, context: PullTypeResolutionContext): PullTypeSymbol {
            ...
            if (aliasedType) {
                if (!aliasedType.isContainer()) {
                    importDecl.addDiagnostic(
                        new SemanticDiagnostic(this.currentUnit.getPath(), importStatementAST.minChar, importStatementAST.getLength(), DiagnosticCode.Module_cannot_be_aliased_to_a_non_module_type, null));
                }
                else {
        ...
        private resolveVariableDeclaration(varDecl: BoundDecl, context: PullTypeResolutionContext, enclosingDecl?: PullDecl): PullSymbol {
            ...
            // Does it have an initializer? If so, typecheck and use that
                ...
                var initExprSymbolAndDiagnostics = this.resolveStatementOrExpression(varDecl.init, false, wrapperDecl, context);
            ...
            if (!returnStatements.length) {
                ...
                for (var i = 0; i < returnStatements.length; i++) {
                    if (returnStatements[i].returnStatement.returnExpression) {
                        returnType = this.resolveStatementOrExpression(returnStatements[i].returnStatement.returnExpression, useContextualType, returnStatements[i].enclosingDecl, context).symbol.getType();
        ...
        public resolveAST(ast: AST, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext): SymbolAndDiagnostics<PullSymbol> {
            switch (ast.nodeType) {
                ...
                case NodeType.VariableDeclarator:
                case NodeType.Parameter:
                    return SymbolAndDiagnostics.fromSymbol(this.resolveDeclaration(ast, context, enclosingDecl));
        ...
        public resolveStatementOrExpression(expressionAST: AST, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext): SymbolAndDiagnostics<PullSymbol> {
            switch (expressionAST.nodeType) {
                ...
                case NodeType.CastExpression:
                    return this.resolveTypeAssertionExpression(<UnaryExpression>expressionAST, inContextuallyTypedAssignment, enclosingDecl, context);
                ...
                case NodeType.AssignmentExpression:
                    return this.resolveAssignmentStatement(<BinaryExpression>expressionAST, inContextuallyTypedAssignment, enclosingDecl, context);
        ...
        private computeDottedNameExpressionSymbol(dottedNameAST: BinaryExpression, enclosingDecl: PullDecl, context: PullTypeResolutionContext): SymbolAndDiagnostics<PullSymbol> {
            ...
            var prevCanUseTypeSymbol = context.canUseTypeSymbol;
            context.canUseTypeSymbol = true;
            var lhs = this.resolveStatementOrExpression(dottedNameAST.operand1, /*inContextuallyTypedAssignment*/false, enclosingDecl, context).symbol;
            context.canUseTypeSymbol = prevCanUseTypeSymbol;
            var lhsType = lhs.getType();
        ...
        private resolveGenericTypeReference(genericTypeAST: GenericType, enclosingDecl: PullDecl, context: PullTypeResolutionContext): SymbolAndDiagnostics<PullTypeSymbol> {
            ...
            if (typeArgs.length && typeArgs.length != typeParameters.length) {
                var diagnostic = new Diagnostic(this.unitPath, genericTypeAST.minChar, genericTypeAST.getLength(), DiagnosticCode.Generic_type__0__requires_1_type_argument_s_, [genericTypeSymbol.toString(), genericTypeSymbol.getTypeParameters().length]);
                var result = this.getNewErrorTypeSymbol(null);
                return SymbolAndDiagnostics.create(result, [diagnostic]);
            }
            ...
            for (var iArg = 0; (iArg < typeArgs.length) && (iArg < typeParameters.length); iArg++) {
                ...
                if (typeConstraint) {
                    ...
                    if (!this.sourceIsAssignableToTarget(typeArg, typeConstraint, context)) {
                        var diagnostic = new Diagnostic(this.unitPath, genericTypeAST.minChar, genericTypeAST.getLength(), DiagnosticCode.Type__0__does_not_satisfy_the_constraint__1__for_type_parameter__2_, [typeArg.toString(true), typeConstraint.toString(true), typeParameters[iArg].toString(true)]);
        ...
        private computeObjectLiteralExpression(expressionAST: AST, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext, additionalResults?: PullAdditionalObjectLiteralResolutionData): SymbolAndDiagnostics<PullSymbol> {
            ...
            if (memberDecls) {
                ...
                for (var i = 0, len = memberDecls.members.length; i < len; i++) {
                    ...
                    var memberExprType = this.resolveStatementOrExpression(binex.operand2, assigningSymbol != null, enclosingDecl, context).symbol;
        ...
        private computeIndexExpressionSymbol(callEx: BinaryExpression, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext): SymbolAndDiagnostics<PullSymbol> {
            ...
            var indexType = this.resolveStatementOrExpression(callEx.operand2, inContextuallyTypedAssignment, enclosingDecl, context).symbol.getType();
        ...
        private resolveLogicalAndExpression(binex: BinaryExpression, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext): SymbolAndDiagnostics<PullSymbol> {
            return SymbolAndDiagnostics.fromSymbol(this.resolveStatementOrExpression(binex.operand2, inContextuallyTypedAssignment, enclosingDecl, context).symbol.getType());
        }
        ...
        public computeCallExpressionSymbol(callEx: CallExpression, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext, additionalResults?: PullAdditionalCallResolutionData): SymbolAndDiagnostics<PullSymbol> {
            // resolve the target
            var targetSymbol = this.resolveStatementOrExpression(callEx.target, inContextuallyTypedAssignment, enclosingDecl, context).symbol;
            ...
            if (!signatures.length && (targetTypeSymbol.getKind() == PullElementKind.ConstructorType)) {
                diagnostics = this.addDiagnostic(diagnostics,
                    context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Value_of_type__0__is_not_callable__Did_you_mean_to_include__new___, [targetTypeSymbol.toString()], enclosingDecl, true));
            }
                                ...
                                for (var j = 0; j < typeParameters.length; j++) {
                                    ...
                                    if (typeConstraint) {
                                        ...
                                        if (!this.sourceIsAssignableToTarget(inferredTypeArgs[j], typeConstraint, context)) {
                                            diagnostics = this.addDiagnostic(diagnostics,
                                                context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Type__0__does_not_satisfy_the_constraint__1__for_type_parameter__2_, [inferredTypeArgs[j].toString(true), typeConstraint.toString(true), typeParameters[j].toString(true)], enclosingDecl));
            ...
            if (!signatures.length) {
                ...
                if (!couldNotFindGenericOverload) {
                    ...
                    diagnostics = this.addDiagnostic(diagnostics, context.postError(this.unitPath, callEx.minChar, callEx.getLength(), DiagnosticCode.Unable_to_invoke_type_with_no_call_signatures, null, enclosingDecl));
                    errorCondition = this.getNewErrorTypeSymbol(diagnostics[0]);
                }
                else {
                    diagnostics = this.addDiagnostic(diagnostics, context.postError(this.unitPath, callEx.minChar, callEx.getLength(), DiagnosticCode.Could_not_select_overload_for__call__expression, null, enclosingDecl));
                    errorCondition = this.getNewErrorTypeSymbol(diagnostics[0]);
                }
            ...
            if (!signature) {
                diagnostics = this.addDiagnostic(diagnostics,
                    context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Could_not_select_overload_for__call__expression, null, enclosingDecl));
        ...
        public computeNewExpressionSymbol(callEx: CallExpression, inContextuallyTypedAssignment: boolean, enclosingDecl: PullDecl, context: PullTypeResolutionContext, additionalResults?: PullAdditionalCallResolutionData): SymbolAndDiagnostics<PullSymbol> {
            ...
            if (constructSignatures.length) {
                ...
                if (!signature) {
                    diagnostics = this.addDiagnostic(diagnostics,
                        context.postError(this.unitPath, targetAST.minChar, targetAST.getLength(), DiagnosticCode.Could_not_select_overload_for__new__expression, null, enclosingDecl));
        ...
        private resolveOverloads(application: AST, group: PullSignatureSymbol[], enclosingDecl: PullDecl, haveTypeArgumentsAtCallSite: boolean, context: PullTypeResolutionContext): PullSignatureSymbol {
            ...
            if (application.nodeType === NodeType.InvocationExpression || application.nodeType === NodeType.ObjectCreationExpression) {
                ...
                if (callEx.arguments) {
                    ...
                    for (var i = 0; i < len; i++) {
                        var argSym = this.resolveStatementOrExpression(callEx.arguments.members[i], false, enclosingDecl, context).symbol;
                ...
                var argSym = this.resolveStatementOrExpression(args.members[0], false, enclosingDecl, context).symbol;
        ...
        private findMostApplicableSignature(signatures: PullApplicableSignature[], args: ASTList, enclosingDecl: PullDecl, context: PullTypeResolutionContext): { sig: PullSignatureSymbol; ambiguous: boolean; } {
            ...
            for (var qSig = 1; qSig < signatures.length; qSig++) {
                ...
                for (var i = 0; args && i < args.members.length; i++) {
                    ...
                    var argSym = this.resolveStatementOrExpression(args.members[i], false, enclosingDecl, context).symbol;
            ...
            for (var i = 0; i < args.members.length; i++) {
                ...
                if (inferenceCandidates.length) {
                    for (var j = 0; j < inferenceCandidates.length; j++) {
                        ...
                        var argSym = this.resolveStatementOrExpression(args.members[i], true, enclosingDecl, context).symbol;
