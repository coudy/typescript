...
module TypeScript2 {
    ...
    export class UpdateUnitResult {
        ...
        public scope1: AST2 = null;
        public scope2: AST2 = null;
        ...
        static singleScopeEdits(script1: Script, script2: Script, scope1: AST2, scope2: AST2, editRange: ScriptEditRange, parseErrors: ErrorEntry[]) {
    ...
    export class TypeScriptCompiler {
        ...
        constructor(public errorOutput: ITextWriter,
                    public logger: ILogger2 = new NullLogger(),
        ...
        public timeFunction(funcDescription: string, func: () => any): any {
            return TypeScript2.timeFunction(this.logger, funcDescription, func);
        }
        ...
        public cleanASTTypesForReTypeCheck(ast: AST2) {
            function cleanASTType(ast: AST2, parent: AST2): AST2 {
                ast.type = null;
            ...
            TypeScript2.getAstWalkerFactory().walk(ast, cleanASTType);
        }
        ...
        public emitUnit(script: Script, reuseEmitter?: bool, emitter?: Emitter) {
            ...
            if (!emitter) {
                ...
                if (this.settings.mapSourceFiles) {
                    emitter.setSourceMappings(new TypeScript2.SourceMapper(fname, outFname, outFile, this.createFile(outFname + SourceMapper.MapFileExtension, false), this.errorReporter, this.settings.emitFullSourceMapPath));
                }
            } else if (this.settings.mapSourceFiles) {
                emitter.setSourceMappings(new TypeScript2.SourceMapper(fname, emitter.emittingFileName, emitter.outfile, emitter.sourceMapper.sourceMapOut, this.errorReporter, this.settings.emitFullSourceMapPath));
            }
        ...
        public pullGetTypeInfoAtPosition(pos: number, script: Script, scriptName?: string): { ast: AST2; typeName: string; typeInfo: string; typeSymbol: PullTypeSymbol; } {
            return this.timeFunction("pullGetTypeInfoAtPosition for pos " + pos + ":", () => {
                ...
                var lastDeclAST: AST2 = null;
                var foundAST: AST2 = null;
                ...
                var objectLitAST: UnaryExpression2 = null;
                var asgAST: BinaryExpression2 = null;
                var typeAssertionASTs: UnaryExpression2[] = [];
                var resolutionContext = new PullTypeResolutionContext();

                var pre = (cur: AST2, parent: AST2): AST2 => {
                        ...
                        if (pos >= cur.minChar && pos <= cur.limChar) {
                            ...
                            if (previous == undefined || (cur.minChar >= previous.minChar && cur.limChar <= previous.limChar)) {
                                ...
                                if (cur.nodeType == NodeType.FuncDecl && hasFlag((<FuncDecl>cur).fncFlags, FncFlags.IsFunctionExpression)) {
                                    ...
                                    objectLitAST = <UnaryExpression2>cur;
                                }
                                else if (cur.nodeType == NodeType.TypeAssertion) {
                                    typeAssertionASTs[typeAssertionASTs.length] = <UnaryExpression2>cur;
                                }
                                else if (cur.nodeType == NodeType.Asg) {
                                    asgAST = <BinaryExpression2>cur;
                                }
                ...
                if (resultASTs.length) {
                    ...
                    if (lastDeclAST == foundAST) {
                        ...
                        if (foundAST.nodeType == NodeType.List) {
                            for (var i = 0; i < (<ASTList2>foundAST).members.length; i++) {
                                if ((<ASTList2>foundAST).members[i].minChar > pos) {
                                    foundAST = (<ASTList2>foundAST).members[i];
                                    break;
                                }
                            }
                        }
